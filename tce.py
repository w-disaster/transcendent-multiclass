...
def main():
    ...
    saved_data_folder = os.path.join(
        "models", "tce-{}-{}".format(args.folds, args.train)
    )
    ...
    loo_val = True if args.folds == X_train.shape[0] or args.folds < 0 else False

    fold_results_list = calibration.train_calibration_set(
        X_train, y_train, args.folds, args.ncpu, saved_data_folder, loo=loo_val
    )
    ...
    if args.thresholds == "quartiles":
    ...
    elif args.thresholds == "full-search":
        scores_p_val_cal = package_cred_conf(
            cred_p_val_cal, conf_p_val_cal, args.criteria
        )

        p_val_found_thresholds = thresholding.full_search_with_constraints(
            scores=scores_p_val_cal,
            predicted_labels=pred_cal,
            groundtruth_labels=groundtruth_cal,
            maximise_vals=args.cs_max,
        ...
        prob_found_thresholds = thresholding.full_search_with_constraints(
            scores=scores_probas_cal,
            predicted_labels=pred_proba_cal,
            groundtruth_labels=groundtruth_cal,
            maximise_vals=args.cs_max,
    ...
    for X_test, y_test in tqdm(test_X_y):
        ...
        if True:
            if args.pval_consider == "full-train":
                ...
                raise ValueError(
                    "Unknown value: args.pval_consider={}".format(args.pval_consider)
                )
        ...
        elif args.thresholds in ("random-search", "constrained-search", "full-search"):
            print_and_extend("=" * 40)
    ...
    data.cache_data(time_series_p_val, "timeseries_cred_conf/tce_p_val_results.p")
    data.cache_data(time_series_proba, "timeseries_cred_conf/tce_proba_results.p")
